...
async function fetchDocuments() {
  ...
  try {
    ...
    console.error('[DocumentUpload] Gagal mengambil daftar dokumen:', err)
...
async function handleFiles(files) {
  ...
  if (pdfFiles.length === 0) {
    uploadError.value = 'Hanya file PDF yang diperbolehkan.'
    return
  }
  ...
  for (const file of pdfFiles) {
    try {
      ...
      uploadError.value = `Gagal mengupload "${file.name}": ${detail}`
  ...
  if (successCount > 0) {
    showSuccess(`${successCount} file berhasil diupload. Daftar dokumen diperbarui.`)
...
async function removeDocument(id) {
  if (!confirm('Hapus dokumen ini dari sistem RAG?')) return
  ...
  try {
    ...
    console.error('[DocumentUpload] Gagal menghapus dokumen:', err)
    fetchDocsError.value =
      err.response?.data?.detail ?? err.message ?? 'Gagal menghapus dokumen.'
  ...
  <div class="space-y-6">
    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            <template v-if="isUploading">Sedang mengupload, harap tunggu…</template>
            <template v-else-if="isDragging">Lepaskan file di sini…</template>
            <template v-else>Drag &amp; drop file PDF atau klik untuk memilih</template>
          </p>
          <p class="text-xs text-slate-400 mt-1">Hanya file PDF • Maksimal 20 MB per file</p>
        </div>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <div>
          <h3 class="text-sm font-semibold text-slate-800">Dokumen SOP Aktif</h3>
          <p class="text-xs text-slate-400 mt-0.5">
            <template v-if="isFetchingDocs">Memuat daftar…</template>
            <template v-else>{{ documents.length }} dokumen tersimpan di sistem RAG</template>
          </p>
        </div>
      ...
      >
        ...
        <button @click="fetchDocuments" class="font-semibold underline hover:no-underline">Coba lagi</button>
      </div>
      ...
      <ul v-else class="divide-y divide-gray-50">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-slate-400 mt-0.5">
              ...
              <template v-if="normDoc(doc, 'pages', 'page_count', null)">
                · {{ normDoc(doc, 'pages', 'page_count') }} halaman
              </template>
              · Diupload {{ normDoc(doc, 'uploadedAt', 'uploaded_at') }}
            </p>
          </div>
        ...
        <li v-if="documents.length === 0 && !isFetchingDocs" class="px-6 py-12 text-center text-slate-400">
          ...
          <p class="text-sm font-medium">Belum ada dokumen yang diupload.</p>
          <p class="text-xs mt-1">Upload file PDF SOP di atas untuk mulai mengisi knowledge base.</p>
        </li>
      </ul>
    </div>
  </div>
