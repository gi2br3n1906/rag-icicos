...
onMounted(async () => {
  try {
    ...
    stats.value[0].delta = 'Terhubung dari database'
    
    stats.value[1].value = data.unique_users.toString()
    stats.value[1].delta = 'Dari total log chat'
    
    stats.value[2].value = data.avg_similarity.toFixed(2)
    stats.value[2].delta = 'Akurasi keseluruhan'
    
    stats.value[3].value = data.active_docs.toString()
    stats.value[3].delta = 'Terindeks ChromaDB'
  } catch (error) {
    console.error('Failed to fetch dashboard stats:', error)
    stats.value.forEach(s => s.delta = 'Gagal memuat data')
  }
})
</script>
  ...
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-sm text-slate-500 mt-1">
        Pantau performa sistem RAG dan riwayat interaksi author ICICoS 2026.
      </p>
    </div>
